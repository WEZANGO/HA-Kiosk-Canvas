...
- **Six font families** installed in the image (DejaVu, Liberation, Noto, Roboto,
  Inconsolata, JetBrains Mono) in regular/bold/italic, offered through a picker
  that reflects what the image has. The editor is served the same font files it
  renders with, so the preview matches the image to within a pixel. Adding a
